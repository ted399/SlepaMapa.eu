...
var presnost = 0;
      ...
      function initMap(link) {
        ...
        var mesta = [];
          ...
          for (var i = 0; i < keys.length; i++) {
            var k = keys[i];
            var nazevlokace = mapa[k].nazevlokace;
            var lat = mapa[k].lat;
            var lng = mapa[k].lng;
            var mesto = new Object();
            mesto.name = nazevlokace;
            mesto.lat = lat;
            mesto.lng = lng;
...
map.setOptions({draggable: false,draggableCursor : "url(img/cursor.png), auto", zoomControl: false, scrollwheel: false, disableDoubleClickZoom: true, disableDefaultUI: true});
